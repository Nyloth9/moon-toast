...
class Dispatcher {
  ...
  constructor(options: DispatcherConstructorArgs) {
    ...
    this.progressBar = {
      ...
      animate: {
        keyframes: options.progressBar?.animate?.keyframes || [
          { width: "100%" },
          { width: "0" },
        ],
  ...
  #toastFactory(
    ...
    return {
      ...
      timer:
        options?.timer ||
        this.#resolveTimer(() => this.dismissToast(id), options?.duration),
      ...
      dissmisOnClick:
        options?.dissmisOnClick ??
        existingToast?.dissmisOnClick ??
        this.dismissOnClick,
      ...
      iconWrapperStyle:
        options?.iconWrapperStyle ||
        existingToast?.iconWrapperStyle ||
  ...
  addCustom: CustomToast = (func, data?) => {
    const args: CustomToastArgs = {
      ...
      timer: this.#resolveTimer(
        () => this.dismissToast(args.id),
        data?.duration,
      ),
    };
  ...
  dismissAllToasts() {
    ...
    this.setStore(
      "toasts",
      produce((toasts) => toasts.forEach((toast) => (toast.state = "exiting"))),
    );
  ...
  setToastRef = (id: string, ref: HTMLDivElement) => {
    ...
    if (currentRef)
      return this.setStore(
        "refArray",
        this.store.refArray.map((storedRef) =>
          storedRef === currentRef ? ref : storedRef,
        ),
      );
  ...
  #preventHeightOverflow() {
    ...
    if (!lastToast) return;
    const lastToastPosition =
      lastToast.getBoundingClientRect()?.top - window.scrollY;
    const lastToastHeight = lastToast.offsetHeight;
    const isToastNearScreenEdge =
      (this.#positionY === "top" &&
        lastToastPosition + lastToastHeight > screenHeight * 0.85) ||
